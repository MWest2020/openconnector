    ...
    <div class="viewContainer">
      ...
      <div class="viewActionsBar">
        ...
        <div class="viewActions">
          <NcActions :force-name="true" :inline="selectedLogs.length > 0 ? 3 : 2" menu-name="Actions">
            <NcActionButton v-if="selectedLogs.length > 0" type="error" close-after-click @click="bulkDeleteLogs">
              <template #icon>
                <Delete :size="20" />
              </template>
      ...
      <div v-else class="viewTableContainer">
        <table class="viewTable callLogsTable">
          ...
          <tbody>
            <tr v-for="log in paginatedLogs" :key="log.id" class="viewTableRow callLogRow" :class="getLogStatusClass(log)">
              ...
              <td class="statusColumn">
                <span class="statusBadge" :class="getLogStatusClass(log)">
                  ...
                  <CloseCircle v-else-if="log.level === 'ERROR'" :size="16" />
...
export default {
  ...
  computed: {
    ...
    filteredLogs() {
      return (synchronizationStore.synchronizationLogs && Array.isArray(synchronizationStore.synchronizationLogs)) ? synchronizationStore.synchronizationLogs : []
    },
    paginatedLogs() {
    ...
    emptyContentName() {
      if (synchronizationStore.loading) {
        return t('openconnector', 'Loading logs...')
      } else if (synchronizationStore.error) {
        return synchronizationStore.error
      } else if (!synchronizationStore.synchronizationLogs?.length) {
  ...
  methods: {
    ...
    getLogStatusClass(log) {
      if (!log.level) return 'unknownStatus'
      if (log.level === 'SUCCESS') return 'successStatus'
      if (log.level === 'WARNING') return 'warningStatus'
      if (log.level === 'ERROR') return 'errorStatus'
      return 'infoStatus'
    },
    viewLogDetails(log) {
...
<style scoped>
/* Copy table and badge styles from SourceLogIndex.vue and adapt as needed */
</style> 